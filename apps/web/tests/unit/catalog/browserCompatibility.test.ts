...
import {
  ...
  getPlatformLabel,
} from "../../../src/lib/browserCompatibility.ts";
...
test("marks native and unsupported libretro systems as desktop-only", () => {
  assert.equal(
    getBrowserGameCompatibility(
      game({ artifact_filename: null, platform_id: "linux", runtime_kind: "native_linux" }),
    ).kind,
    "desktop",
  );
  assert.equal(
    getBrowserGameCompatibility(
      game({ artifact_filename: "game.sfc", platform_id: "snes", runtime_id: "bsnes" }),
    ).kind,
    "desktop",
  );
});
...
test("marks missing verification metadata as unavailable", () => {
  ...
  assert.equal(
    getBrowserGameCompatibility({ cover_url: "", id: "none", title: "None" }).kind,
    "unavailable",
  );
});
