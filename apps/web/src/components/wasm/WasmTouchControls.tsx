...
export function WasmTouchControls({ gameKey, onPress, onRelease, status }: Props) {
  ...
  const [initialPreferences] = useState(() => readTouchControlPreferences(preferenceKey));
  ...
  const tone =
    preset === "contrast"
      ? "border-yellow-300 bg-black text-yellow-200"
    ...
    <details className="border-t border-synth-border bg-synth-surface p-3 md:hidden">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-3 text-xs text-gray-300">
        <label>
          Preset{" "}
          <select
            className="ml-1 rounded border border-synth-border bg-synth-bg p-1.5"
            onChange={(event) => setPreset(event.target.value as TouchControlPreset)}
            value={preset}
          >
        ...
        <label className="flex items-center gap-1.5">
          <input
            checked={swapButtons}
            onChange={(event) => setSwapButtons(event.target.checked)}
            type="checkbox"
          />{" "}
          Swap A/B layout for this game
        </label>
      </div>
      <div className="mt-4 flex items-center justify-between gap-4">
        <div className="grid grid-cols-3 gap-1" aria-label="Directional pad">
          <span />
          <button aria-label="Up" className={controlClass} disabled={!enabled} {...bind("up")}>
            ▲
          </button>
          <span />
          <button aria-label="Left" className={controlClass} disabled={!enabled} {...bind("left")}>
            ◀
          </button>
          <span />
          <button
          ...
          <span />
          <button aria-label="Down" className={controlClass} disabled={!enabled} {...bind("down")}>
            ▼
          </button>
          <span />
        </div>
        <div className="flex items-end gap-2">
          <button className={controlClass} disabled={!enabled} {...bind("select")}>
            Select
          </button>
          <button className={controlClass} disabled={!enabled} {...bind("start")}>
            Start
          </button>
          <button className={controlClass} disabled={!enabled} {...bind(swapButtons ? "a" : "b")}>
            {swapButtons ? "A" : "B"}
          </button>
          <button className={controlClass} disabled={!enabled} {...bind(swapButtons ? "b" : "a")}>
            {swapButtons ? "B" : "A"}
          </button>
        </div>
      </div>
