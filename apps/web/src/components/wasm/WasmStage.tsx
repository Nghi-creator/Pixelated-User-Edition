...
export function WasmStage({
  ...
  return (
    <div
      className="relative aspect-[4/3] w-full overflow-hidden bg-black fullscreen:h-screen fullscreen:w-screen fullscreen:aspect-auto"
      ref={stageRef}
    >
      <canvas
      ...
      {loadingLabel && (
        <div
          className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-black/80 text-gray-200"
          role="status"
        >
      ...
      {status === "paused" && (
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center bg-black/40">
          <span className="rounded-md bg-black/70 px-5 py-3 font-bold uppercase tracking-wider text-white">
            Paused
          </span>
        </div>
      )}
