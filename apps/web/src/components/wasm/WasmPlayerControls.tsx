...
export function WasmPlayerControls({
  ...
  return (
    <div
      ref={controlsRef}
      className="relative z-30 flex min-h-14 w-full items-center gap-2 border-b border-synth-border bg-synth-surface px-3 py-2"
    >
      ...
      <div className="hidden h-10 items-center rounded-lg border border-[#5D263A] bg-[#351B27] sm:flex">
        <button
          ...
          className="inline-flex h-full w-10 items-center justify-center border-r border-[#5D263A] text-white hover:bg-[#2B1720]"
        ...
        <label className="flex h-full items-center px-3">
          ...
          <input
            ...
            className="w-20 accent-synth-primary sm:w-24"
            max="1"
            min="0"
      ...
      <div className="relative">
        <button
          aria-controls="wasm-player-settings-panel"
        ...
        {isSettingsOpen && (
          <div
            id="wasm-player-settings-panel"
            className="absolute right-0 top-full mt-3 w-[min(22rem,calc(100vw-2rem))] rounded-lg border border-synth-border bg-synth-surface/95 p-4 text-left shadow-panel backdrop-blur-md"
          >
            <span
              aria-hidden="true"
            ...
            <div className="relative mt-2 grid grid-cols-2 gap-2">
              <button
                className={buttonClass}
                onClick={() => openPlayerTool(onOpenInputSettings)}
                type="button"
              >
                Keyboard
              </button>
              <button
                className={buttonClass}
                onClick={() => openPlayerTool(onOpenSaveStates)}
                type="button"
              >
                Local states
              </button>
