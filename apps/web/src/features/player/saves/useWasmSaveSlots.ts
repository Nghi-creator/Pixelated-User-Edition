...
export function useWasmSaveSlots({ captureState, gameKey, restoreState, runtime }: Options) {
  ...
  useEffect(() => { void refresh().catch(() => setMessage("Browser save storage is unavailable.")); }, [refresh]);
  const run = useCallback(async (slot: WasmSaveSlot, action: () => Promise<string>) => {
    setBusySlot(slot); setMessage(null);
    try { setMessage(await action()); await refresh(); }
    catch (error) { setMessage(error instanceof Error ? error.message : "The save operation failed."); }
    finally { setBusySlot(null); }
  }, [refresh]);
  const save = (slot: WasmSaveSlot) => run(slot, async () => {
    const captured = await captureState();
    await putWasmSaveRecord(createWasmSaveRecord(gameKey, slot, captured.state, runtime, captured.thumbnail));
    return `Saved slot ${slot}.`;
  });
  const load = (slot: WasmSaveSlot, record: WasmSaveRecord) => run(slot, async () => {
    if (record.core !== runtime.core || record.system !== runtime.system) throw new Error("This save was created by a different emulator core.");
    await restoreState(record.state); return `Loaded slot ${slot}.`;
  });
  const importFile = (slot: WasmSaveSlot, file: File) => run(slot, async () => {
    validateImportedState(file); await putWasmSaveRecord(createWasmSaveRecord(gameKey, slot, file, runtime));
    return `Imported into slot ${slot}. Start the same game before loading it.`;
  });
  const remove = (slot: WasmSaveSlot) => run(slot, async () => {
    await deleteWasmSaveRecord(gameKey, slot); return `Deleted slot ${slot}.`;
  });
