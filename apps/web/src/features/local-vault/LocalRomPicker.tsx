...
export function LocalRomPicker({ fileInputVersion, isInspecting, onSelectFile }: Props) {
  ...
  return (
    <div
      ...
      onDrop={handleDrop}
    >
      <input accept={ACCEPTED_ROM_EXTENSIONS} className="absolute inset-0 z-10 h-full w-full cursor-pointer opacity-0" disabled={isInspecting} key={fileInputVersion} onChange={handleInput} type="file" />
