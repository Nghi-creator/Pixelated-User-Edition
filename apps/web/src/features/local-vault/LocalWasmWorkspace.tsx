...
import { PlayerHeader, type PlayerHeaderStatus } from "../player/components/PlayerHeader";
import { PlayerStreamGrid } from "../player/components/PlayerStreamGrid";
import { WasmInputSettings } from "../player/components/WasmInputSettings";
import { WasmPlayerControls } from "../player/components/WasmPlayerControls";
import { WasmPlayerToolDrawer } from "../player/components/WasmPlayerToolDrawer";
import { WasmResearchPanel } from "../player/components/WasmResearchPanel";
import { WasmSavePanel } from "../player/components/WasmSavePanel";
import { WasmStage } from "../player/components/WasmStage";
import { WasmTouchControls } from "../player/components/WasmTouchControls";
import { useWasmResearch } from "../player/hooks/useWasmResearch";
...
export function LocalWasmWorkspace({ file, fileInputVersion, isInspecting, onClose, onSelectFile, system }: Props) {
  ...
  return (
    <div className="flex min-h-screen flex-col items-center px-4 pb-24 pt-24">
      <PlayerHeader backRoute="/local" backText="Personal ROMs" gameTitle={file.name} hideGameChrome layoutClassName={layoutClassName} onBack={closeWorkspace} onToggleTelemetry={() => setShowBrowserTelemetry((visible) => !visible)} showStreamTelemetry={showBrowserTelemetry} status={headerStatus} statusLabelOverride={statusLabels[player.status]} />
      <PlayerStreamGrid layoutClassName={layoutClassName} showStreamTelemetry={showBrowserTelemetry} telemetryPanel={<WasmResearchPanel onClose={() => setShowBrowserTelemetry(false)} research={research} variant="sidebar" />}>
