...
export function LocalWasmWorkspace({
  file,
  fileInputVersion,
  isInspecting,
  onClose,
  ...
  const headerStatus: PlayerHeaderStatus =
    player.status === "playing" || player.status === "paused"
      ...
      : player.status === "error"
        ? "error"
        : ["preparing", "downloading", "verifying", "loading-core", "starting"].includes(
              player.status,
            )
          ? "connecting"
          : "idle";
  ...
  return (
    <div className="flex min-h-screen flex-col items-center px-4 pb-24 pt-24">
      <PlayerHeader
        ...
        onToggleTelemetry={() => setShowBrowserTelemetry((visible) => !visible)}
      ...
      <div className={`mt-3 flex w-full ${layoutClassName} justify-end`}>
        ...
        />
        <button
          className="rounded-md border border-synth-border bg-synth-surface px-4 py-2 text-sm font-bold text-white hover:bg-synth-elevated disabled:opacity-50"
          disabled={isInspecting}
