...
import type { ProfileActivityState } from "./ProfileSettingsState";
import { getGamePlayPath } from "../../lib/appUrl";

export function RecentActivitySection({ profile }: { profile: ProfileActivityState }) {
  if (profile.loading)
    return (
      ...
      <div className="mb-5 flex items-center justify-between gap-4">
        <div>
          <h2 className="flex items-center gap-2 text-xl font-bold text-white">
            <Clock3 className="h-5 w-5 text-synth-primary" />
            Recent Activity
          </h2>
      ...
      {profile.error ? (
        ...
        <div className="space-y-3">
          {profile.entries.map((entry) => (
            <Link
              className="group flex items-center gap-4 rounded-lg border border-synth-border bg-synth-bg/50 p-3 hover:bg-synth-elevated"
              key={`${entry.game_id}:${entry.client_edition}:${entry.runtime_kind}`}
              state={{ backRoute: "/profile", backText: "Back to Account Settings" }}
              to={getGamePlayPath(entry.game_id)}
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-400">
                  <span className="inline-flex items-center gap-1">
                    ...
                    {entry.client_edition === "user" ? "User Edition" : "Studio Edition"} ·{" "}
                    {entry.runtime_kind.toUpperCase()}
                  </span>
                  <span className="inline-flex items-center gap-1">
              ...
              <time
                className="hidden text-right text-xs text-gray-500 sm:block"
                dateTime={entry.last_played_at}
              >
                {new Intl.DateTimeFormat(undefined, { dateStyle: "medium" }).format(
                  new Date(entry.last_played_at),
                )}
              </time>
            </Link>
          ))}
