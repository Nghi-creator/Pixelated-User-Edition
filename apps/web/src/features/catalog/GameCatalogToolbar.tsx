...
import { PLATFORM_OPTIONS } from "../../lib/catalog/browserCompatibility";
import { formatGenre } from "./catalogMetadata";

type RuntimeFilter = "all" | "browser" | "desktop" | "unavailable";
type Props = {
  availableGenres: string[]; availableLicenses: string[]; genre: string; hasActiveFilters: boolean;
  license: string; onGenreChange: (value: string) => void; onLicenseChange: (value: string) => void;
  onPlatformChange: (value: string) => void; onReset: () => void; onRuntimeChange: (value: RuntimeFilter) => void;
  onSearchChange: (value: string) => void; platform: string; runtime: RuntimeFilter; search: string;
};
  ...
  return (
    <div className="mb-8 space-y-3">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <h2 className="scroll-mt-24 text-2xl font-bold text-white" id="all-games">All Games</h2>
        <div className="grid w-full xl:max-w-4xl xl:grid-cols-4"><div className="relative w-full xl:col-span-2 xl:col-start-3"><Search className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-gray-400" /><input className="block w-full rounded-lg border border-synth-border bg-synth-bg py-2 pl-10 pr-3 text-white placeholder:text-gray-500 focus:border-synth-secondary focus:outline-none" onChange={(event) => props.onSearchChange(event.target.value)} placeholder="Search games..." type="text" value={props.search} /></div></div>
      </div>
