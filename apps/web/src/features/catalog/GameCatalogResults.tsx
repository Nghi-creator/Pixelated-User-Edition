...
export function GameCatalogResults(props: Props) {
  ...
  if (showInitialSkeleton) content = <GamesCatalogSkeleton />;
    ...
    content = (
      <CatalogRefreshPanel label={props.searchQuery ? "Searching games..." : "Loading games..."} />
    );
  else if (props.isError)
    content = (
      <div className="danger-panel rounded-lg border px-4 py-8 text-center font-bold">
        <p>Could not load the game library. Check the API connection.</p>
        <button
    ...
    content = (
      <div className="py-20 text-center text-gray-500">
        ...
        <p className="text-xl">
          {props.searchQuery
            ? `No games found matching “${props.searchQuery}” with these filters.`
  ...
  return (
    <>
      {content}
      {!props.isError && props.games.length > 0 && props.totalPages > 1 && (
        <div className="mt-10 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm text-gray-500">
            Showing {props.pageStart + 1}-
            {Math.min(props.pageStart + props.games.length, props.totalGames)} of {props.totalGames}
          </p>
          <Pagination
