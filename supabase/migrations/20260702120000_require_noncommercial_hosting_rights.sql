-- Make non-commercial hosting approval explicit in rights review.
--
-- Existing verified rights were reviewed under the current non-commercial
-- hosting policy, so backfill them as allowed. New verified rights must carry
-- this explicit decision before they can make a build public.

ALTER TABLE public.game_rights
  ADD COLUMN IF NOT EXISTS noncommercial_hosting_allowed boolean;

ALTER TABLE public.catalog_ingestion_candidates
  ADD COLUMN IF NOT EXISTS permission_evidence_url text,
  ADD COLUMN IF NOT EXISTS noncommercial_hosting_allowed boolean;

UPDATE public.game_rights
SET noncommercial_hosting_allowed = true
WHERE verified_at IS NOT NULL
  AND noncommercial_hosting_allowed IS NULL;
...
ALTER TABLE public.game_rights
  DROP CONSTRAINT IF EXISTS game_rights_verified_noncommercial_required;

ALTER TABLE public.game_rights
  ADD CONSTRAINT game_rights_verified_noncommercial_required CHECK (
    verified_at IS NULL
    OR noncommercial_hosting_allowed IS TRUE
  );
...
USING (
  ...
  AND noncommercial_hosting_allowed IS TRUE
  AND EXISTS (
    SELECT 1
    FROM public.games
